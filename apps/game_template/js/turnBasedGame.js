...
SocialKit.Multiplayer.TurnBasedMultiplayerGame.prototype.isMyTurn = function() {
    if (DBG) console.info("checking turn " + this.players + ", " + this._currentPlayerIdx);
    return (this.players[this._currentPlayerIdx].id == this.appContext.user.id);
};

//Returns the ID of the player controlled by the device asking. Useful for debugging.
SocialKit.Multiplayer.TurnBasedMultiplayerGame.prototype.whoAmI = function () {
    for(var idx = 0; idx < this.players.length; idx ++) {
        if(this.players[idx].id == this.appContext.user.id) return idx;
    }
    return -1;
};

//Returns the ID of the player who is in control of the state machine.
SocialKit.Multiplayer.TurnBasedMultiplayerGame.prototype.whoseTurn = function() {
...
SocialKit.Multiplayer.TurnBasedMultiplayerGame.prototype.takeTurn = function(state, nextPlayer) {
    ...
    if (DBG) console.info("it's my turn");
    this.state = state;
    this._lastTurn++;
